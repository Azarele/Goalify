/*
  ...
  2. Database Functions
    ...
    - complete_goal_with_xp: Goal completion with XP rewards
    - get_user_goals: Filtered goal retrieval
    - create_user_goal: Goal creation with validation

  3. Security
    - Update RLS policies for proper access control
    - Create view for goal summaries with RLS
...
ALTER VIEW user_goal_summary SET (security_barrier = true);

-- Create RLS policy for the view
CREATE POLICY "Users can view goal summaries for leaderboard"
  ON user_goal_summary
  FOR SELECT
  TO authenticated
  USING (true);