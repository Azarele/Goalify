/*
  ...
  1. New Functions
    - `sync_user_data` - Synchronizes user data across devices
    - `get_sync_status` - Gets the sync status for a user
    - `mark_data_for_sync` - Marks data for synchronization
  
  2. Improved Indexes
    - Added indexes for better sync performance
    - Added last_synced column to relevant tables
  
  3. Security
    - Enhanced RLS policies for cross-device access
    - Added sync-specific permissions
*/
...
-- Enhance the goal completion function to handle sync
...
-- Enhance the create goal function to handle sync
