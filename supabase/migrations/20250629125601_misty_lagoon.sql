/*
  # Enhanced Goals System with Database Functions

  1. Schema Updates
    - Make session_id nullable in goals table
    - Add performance indexes for goals queries
    - Add foreign key constraint for user_id

  2. Database Functions
    - get_user_goal_stats: Comprehensive goal statistics
    - complete_goal_with_xp: Complete goals with XP calculation
    - get_user_goals: Retrieve user goals with filtering
    - create_user_goal: Create new goals with validation

  3. Security
    - Update RLS policies for goals table
    - Create view for goal summaries (accessible to all authenticated users)

  4. Performance
    - Add indexes for common query patterns
    - Optimize goal retrieval and statistics
*/
...
DROP FUNCTION IF EXISTS get_user_goal_stats(uuid);
...
CREATE FUNCTION get_user_goal_stats(target_user_id uuid)
RETURNS TABLE (
  total_goals integer,
...
CREATE FUNCTION complete_goal_with_xp(
  goal_id uuid,
...
CREATE FUNCTION get_user_goals(
  target_user_id uuid,
...
CREATE FUNCTION create_user_goal(
...
DROP VIEW IF EXISTS user_goal_summary;

-- Create a view for easy goal statistics access
-- Note: Views inherit security from underlying tables, so no separate RLS needed
...
SELECT 
  ...
  COALESCE(goal_stats.pending_goals, 0) as pending_goals,
  COALESCE(goal_stats.overdue_goals, 0) as overdue_goals,
  COALESCE(goal_stats.total_xp_earned, 0) as total_xp_earned_from_goals,
  COALESCE(goal_stats.completion_rate, 0) as completion_rate
FROM user_profiles up
LEFT JOIN LATERAL get_user_goal_stats(up.user_id) goal_stats ON true;

-- Grant access to the view for authenticated users
GRANT SELECT ON user_goal_summary TO authenticated;